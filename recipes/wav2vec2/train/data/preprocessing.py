...
class AudioProcessingPipeline:
    ...
    def add_path_resolution(
        ...
        """
        Add audio path resolution to pipeline.
        
        ORIGINAL EQUIVALENT: fairseq2:e9fbd6/src/fairseq2/datasets/speech.py:328-336
        In: add_audio_decoding() -> FileMapper logic
        
        NEW IMPLEMENTATION: FileMapper doesn't exist in v0.5, so we resolve paths manually
        NOTE: cached_fd_count functionality removed - not available in v0.5 AudioDecoder
        """
        # Memory map audio files - ORIGINAL: line 328-330
        cached_fd_count = extras.get("cached_fd_count", 1000)
        # NOTE: cached_fd_count cannot be replicated in v0.5 - AudioDecoder handles caching internally
        
        if audio_dir is not None:
            def resolve_audio_path(example):
                example["audio"] = {"path": audio_dir / example["audio"]}  # Match original structure
                return example
            builder.map(resolve_audio_path)
        else:
            def copy_audio_path(example):
                example["audio"] = {"path": Path(example["audio"])}  # Match original structure
                return example
            builder.map(copy_audio_path)
    ...
    def add_audio_decoding(
        ...
        builder.map(
            audio_decoder, selector="[*].audio.data", num_parallel_calls=npc  # ORIGINAL: line 341-344
        )
        return builder
