...
class Wav2Vec2TrainDataset:
    ...
    def create_reader(
        ...
        *,
        ...
        if extras is None:  # ORIGINAL: line 534-535
            extras = {}
        ...
        seed = seed  # ORIGINAL: line 542 - explicit seed initialization
        no_padding = no_padding  # ORIGINAL: line 543
        ...
        builder = pipeline.add_path_resolution(builder, audio_dir, extras)
        ...
        builder = pipeline.add_audio_cropping(
            builder, seed, max_audio_len, crop_to_batch_minimal_size=not no_padding
        )
...
class Wav2Vec2TrainDatasetSection(DatasetSection):
    ...
    data: Path | None = (
        None  # ORIGINAL: line 124 # TODO: (cirquit) - add to changelog from `path` to `data`?
    )
    ...
    batch_size: int | None = None  # needed for BatchingStrategy.STATIC
    max_num_elements: int = (
        1_500_000  # ORIGINAL: line 138 # needed for BatchingStrategy.LENGTH
    )
    ...
    )  # ORIGINAL: DataReadOptions line 103
...
def open_wav2vec2_train_dataset(
    ...
    return Wav2Vec2TrainDataset.from_path(config.path, name)
